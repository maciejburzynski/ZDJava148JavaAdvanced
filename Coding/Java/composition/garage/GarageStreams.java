...
public class GarageStreams {
    public static void main(String[] args) {
        ...
        // printing heated garages using streams
        List<Integer> heatedGarage = garages
                .stream()
                .filter(Garage::isHeated)
                .map(Garage::getGarageNum)
                .collect(Collectors.toList());

        System.out.println(heatedGarage);
        // printing garages which contains summary value of lumpers over 1500
        List<Integer> mostValuableGarages = garages
                .stream()
                .filter(garage -> garage.getSummaryLumpersValue() > 1500)
                .map(Garage::getGarageNum)
                .collect(Collectors.toList());

        System.out.println(mostValuableGarages);
        // printing garages which 2 parking spaces or more
        List<Integer> garagesWithTwoParkingSpacesOrMore = garages
                .stream()
                .filter(garage -> garage.getSummaryLumpersValue() > 1500)
                .map(Garage::getGarageNum)
                .collect(Collectors.toList());
